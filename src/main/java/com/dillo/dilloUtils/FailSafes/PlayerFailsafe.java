...
public class PlayerFailsafe {
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.END && config.playerFailsafe && !MinecraftServer.getServer().isSinglePlayer() && isPlayerOnServer() && ArmadilloStates.offlineState == "online") {
            if (ids.mc.theWorld != null) {
                ...
                for (Object playerObj : ids.mc.theWorld.playerEntities) {
                    ...
                    if (players.contains(player.getName())) {
                        if (DistanceFromTo.distanceFromTo(player.getPosition(), ids.mc.thePlayer.getPosition()) < config.playerFailsafeRange) {
