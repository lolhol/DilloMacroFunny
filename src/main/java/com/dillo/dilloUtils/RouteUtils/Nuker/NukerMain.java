...
public class NukerMain {
            ...
            if (nuking.size() > 0) {
                if (System.currentTimeMillis() >= lastTime + (1000 / config.nukerBPS)) {
                    ...
                    if (!isAbleToMine(block)) {
                        ...
                        if (isInFOV(block, config.nukerFOV) && isHoldingDrill() && DistanceFromTo.distanceFromTo(ids.mc.thePlayer.getPosition(), block) < 4.5 && isOnGround() && isAbleToMine(block) && isInDillo()) {
    ...
    public static boolean isHoldingDrill() {
        ...
        if (name != null) {
            ...
            if (name != null) {
                if (stringName.toLowerCase().contains("drill") || stringName.toLowerCase().contains("pickaxe") || stringName.toLowerCase().contains("gauntlet")) {
                    return true;
                }
            }
        }
    ...
    public static Float getYawBlock(BlockPos block) {
        ...
        if (ids.mc.thePlayer.rotationYaw > 360) {
            playerYaw = (float) (((ids.mc.thePlayer.rotationYaw / 360) - (Math.floor(ids.mc.thePlayer.rotationYaw / 360))) * 360);
        }
    ...
    public static boolean canBeBroken(BlockPos block) {
        ...
        if ((result.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK || result.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY) && result.hitVec != endVec) {
            // Obstruction found
            return true;
        }
