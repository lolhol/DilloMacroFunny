package com.dillo.dilloUtils.RouteUtils.ViewClearLines;
        ...
        if (isRender) {
            for (int i = 0; i < routeBlocks.size(); i++) {
                ...
                if (block1 != null && block2 != null) {
                    RenderLine.drawLine(new Vec3(block1.getX() + 0.5, block1.getY() + 2.5 + ids.mc.thePlayer.getEyeHeight(), block1.getZ() + 0.5), new Vec3(block2.getX()+ 0.5, block2.getY() + 0.5 + ids.mc.thePlayer.getEyeHeight(), block2.getZ()+ 0.5), 1F, Color.white, event.partialTicks);
                }
            }
        }
    }
