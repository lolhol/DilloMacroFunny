...
public class GetBlocksForNuker {
        ...
        new Thread(() -> {
            ...
            for (int i = 0; i < blocksOnRoute.size(); i++) {
                ...
                if (second < blocksOnRoute.size()) {
                    ...
                    blocks.addAll(findBlocks(new Vec3(block.getX(), block.getY() + 1.64, block.getZ()), BlockUtils.fromBlockPosToVec3(blocksOnRoute.get(second)), 1));
                }
            }
    ...
    public static List<BlockPos> findBlocks(Vec3 pos1, Vec3 pos2, double cylRad) {
        ...
        for (int i = 0; i < length; i++) {
            for (int degree = 0; degree < 360; degree += 10) {
                ...
                for (int j = 0; j < finalBlockPos.size() - 1; j++) {
                    if (finalBlockPos.get(j).getX() == block.getX() && finalBlockPos.get(j).getY() == block.getY() && finalBlockPos.get(j).getZ() == block.getZ()
                    ) {
                        found = true;
                    }
                }
        ...
        finalBlockPos.sort((a, b) -> {
            if (DistanceFromTo.distanceFromTo(a, BlockUtils.fromVec3ToBlockPos(pos1)) < DistanceFromTo.distanceFromTo(b, BlockUtils.fromVec3ToBlockPos(pos1))) {
                return -1;
            } else if (DistanceFromTo.distanceFromTo(a, BlockUtils.fromVec3ToBlockPos(pos1)) == DistanceFromTo.distanceFromTo(b, BlockUtils.fromVec3ToBlockPos(pos1))) {
                return 0;
            } else {
                return 1;
            }
