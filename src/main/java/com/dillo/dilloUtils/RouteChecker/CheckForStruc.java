...
public class CheckForStruc {
    ...
    private static boolean isNaturalBlock(BlockPos block) {
        ...
        return blockType != Blocks.prismarine && blockType != Blocks.stained_glass && blockType != Blocks.stained_glass_pane && blockType != Blocks.stone && blockType != Blocks.coal_ore && blockType != Blocks.emerald_ore && blockType != Blocks.diamond_ore && blockType != Blocks.gold_ore && blockType != Blocks.iron_ore && blockType != Blocks.lapis_ore && blockType != Blocks.lit_redstone_ore && blockType != Blocks.redstone_ore;
    }
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = 0; j <= 4; j++) {
                for (int k = -1; k <= 1; k++) {
                    ...
                    if (ids.mc.theWorld.getBlockState(block).getBlock() == Blocks.stained_glass_pane || ids.mc.theWorld.getBlockState(block).getBlock() == Blocks.stained_glass) {
                        glassCount++;
                    }
