...
public class RenderUtils {
  ...
  public static void renderTracer(
    double posX,
    double posY,
    double posZ,
    double height,
    ...
    worldRenderer
      .pos(realX, realY + render.getEyeHeight(), realZ)
  ...
  public static void renderWaypointText(
    String str,
    double X,
    double Y,
    double Z,
  ...
  public static void drawLine(Vec3 from, Vec3 to, final float thickness, final float partialTicks) {
    ...
    bufferBuilder
      .pos(from.xCoord, from.yCoord, from.zCoord)
      .color((i >> 16 & 0xFF) / 255.0f, (i >> 8 & 0xFF) / 255.0f, (i & 0xFF) / 255.0f, (i >> 24 & 0xFF) / 255.0f)
      .endVertex();
    bufferBuilder
      .pos(to.xCoord, to.yCoord, to.zCoord)
      .color((i >> 16 & 0xFF) / 255.0f, (i >> 8 & 0xFF) / 255.0f, (i & 0xFF) / 255.0f, (i >> 24 & 0xFF) / 255.0f)
      .endVertex();
  ...
  public static void drawLines(ArrayList<Vec3> poses, final float thickness, final float partialTicks) {
    ...
    for (final Vec3 pos : poses) {
      ...
      bufferBuilder
        .pos(pos.xCoord + 0.5, pos.yCoord + 0.5, pos.zCoord + 0.5)
        .color((i >> 16 & 0xFF) / 255.0f, (i >> 8 & 0xFF) / 255.0f, (i & 0xFF) / 255.0f, (i >> 24 & 0xFF) / 255.0f)
        .endVertex();
    }
    Tessellator.getInstance().draw();
  ...
  public static void renderEspBox(BlockPos blockPos, float partialTicks, int color, float opacity) {
    if (blockPos != null) {
      ...
      if (blockState != null) {
        ...
        double d0 =
          ids.mc.thePlayer.lastTickPosX +
          (ids.mc.thePlayer.posX - ids.mc.thePlayer.lastTickPosX) *
          (double) partialTicks;
        double d1 =
          ids.mc.thePlayer.lastTickPosY +
          (ids.mc.thePlayer.posY - ids.mc.thePlayer.lastTickPosY) *
          (double) partialTicks;
        double d2 =
          ids.mc.thePlayer.lastTickPosZ +
          (ids.mc.thePlayer.posZ - ids.mc.thePlayer.lastTickPosZ) *
          (double) partialTicks;
        drawFilledBoundingBox(
          block.getSelectedBoundingBox(ids.mc.theWorld, blockPos).expand(0.002D, 0.002D, 0.002D).offset(-d0, -d1, -d2),
          color,
          opacity
        );
      }
  ...
  public static void renderAxisSquare(
    BlockPos blockPos,
