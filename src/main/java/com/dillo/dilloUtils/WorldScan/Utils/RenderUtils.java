package com.dillo.dilloUtils.WorldScan.Utils;
...
import com.dillo.dilloUtils.WorldScan.Utils.objects.Shader;
...
public class RenderUtils {
    ...
    public static void renderEntityModel(Entity entity, Vec3 vec, float partialTicks, float opacity) {
        ...
        ids.mc.getRenderManager().doRenderEntity(entity, x, y, z, yaw, partialTicks, false);
    }

    public static void renderTracer(double posX, double posY, double posZ, double height, Color color, float partialTicks) {
        Entity render = ids.mc.getRenderViewEntity();
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldRenderer = tessellator.getWorldRenderer();
    ...
    public static void renderBeacon(double x, double y, double z, Color color, float partialTicks) {
        ...
        GlStateManager.pushMatrix();
        GlStateManager.disableDepth();
        GlStateManager.translate(-ids.mc.getRenderManager().viewerPosX, -ids.mc.getRenderManager().viewerPosY, -ids.mc.getRenderManager().viewerPosZ);
        ids.mc.getTextureManager().bindTexture(beaconBeam);
        ...
        double time = ids.mc.theWorld.getWorldTime() + partialTicks;
    ...
    public static void drawLine(Vec3 from, Vec3 to, final float thickness, final float partialTicks) {
        final Entity render = ids.mc.getRenderViewEntity();
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer bufferBuilder = tessellator.getWorldRenderer();
        final double realX = render.lastTickPosX + (render.posX - render.lastTickPosX) * partialTicks;
        final double realY = render.lastTickPosY + (render.posY - render.lastTickPosY) * partialTicks;
    ...
    public static void renderEspBox(BlockPos blockPos, float partialTicks, int color, float opacity) {
        if (blockPos != null) {
            ...
            if (blockState != null) {
                ...
                block.setBlockBoundsBasedOnState(ids.mc.theWorld, blockPos);
                double d0 = ids.mc.thePlayer.lastTickPosX + (ids.mc.thePlayer.posX - ids.mc.thePlayer.lastTickPosX) * (double) partialTicks;
                double d1 = ids.mc.thePlayer.lastTickPosY + (ids.mc.thePlayer.posY - ids.mc.thePlayer.lastTickPosY) * (double) partialTicks;
                double d2 = ids.mc.thePlayer.lastTickPosZ + (ids.mc.thePlayer.posZ - ids.mc.thePlayer.lastTickPosZ) * (double) partialTicks;
                drawFilledBoundingBox(block.getSelectedBoundingBox(ids.mc.theWorld, blockPos).expand(0.002D, 0.002D, 0.002D).offset(-d0, -d1, -d2), color, opacity);
            }
        }
    }
    ...
    public static void renderSmallBox(Vec3 vec, int color, float opacity) {
        RenderManagerAccessor renderManager = (RenderManagerAccessor) ids.mc.getRenderManager();
    ...
    public static void renderAxisSquare(BlockPos blockPos, EnumFacing enumFacing, float partialTicks, int color, float opacity) {
        if (blockPos != null) {
            ...
            if (blockState != null) {
                ...
                double d1 = ids.mc.thePlayer.lastTickPosY + (ids.mc.thePlayer.posY - ids.mc.thePlayer.lastTickPosY) * (double) partialTicks;
                double d2 = ids.mc.thePlayer.lastTickPosZ + (ids.mc.thePlayer.posZ - ids.mc.thePlayer.lastTickPosZ) * (double) partialTicks;
                drawFilledAxisSquare(block.getSelectedBoundingBox(ids.mc.theWorld, blockPos).expand(0.002D, 0.002D, 0.002D).offset(-d0, -d1, -d2), enumFacing, color, opacity);
            }
        }
    }
