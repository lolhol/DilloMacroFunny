    ...
    public static List<BlockPos> getBlocksLayer(BlockPos refrenceBlock) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                ...
                if ((ids.mc.theWorld.getBlockState(block).getBlock() == Blocks.stained_glass_pane || ids.mc.theWorld.getBlockState(block).getBlock() == Blocks.stained_glass) && !isSameBlock(block, refrenceBlock)) {
                    blocks.add(block);
                }
            }
        }
