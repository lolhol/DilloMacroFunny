...
public class ActionMineUp extends Action {
    ...
    private abstract static class MineUpActionFactory implements ActionFactory {
        ...
        Result check(Node node, Direction direction) {
            ...
            return Result.valid(
                    direction,
                    NodeCache.get(to),
                    totalTicksTopBreak + ActionCosts.get().STEP_UP_STRAIGHT + ActionCosts.get().CONSTANT_BLOCK_BREAK_MOD,
                    modifications
            );
        }
    }

    private abstract static class AbstractMineUpActionFactory extends MineUpActionFactory {
