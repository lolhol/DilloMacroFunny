...
import com.dillo.pathfinding.stevebot.core.pathfinding.actions.*;
...
public class ActionMineStraight extends Action {
    ...
    private abstract static class MineStraightActionFactory implements ActionFactory {
        ...
        Result check(Node node, Direction direction) {
            ...
            return Result.valid(
                    direction,
                    NodeCache.get(to),
                    totalTicksTopBreak + ActionCosts.get().WALK_SPRINT_STRAIGHT + ActionCosts.get().CONSTANT_BLOCK_BREAK_MOD,
                    modifications
            );
        }
    }

    private abstract static class AbstractMineStraightActionFactory extends MineStraightActionFactory {
