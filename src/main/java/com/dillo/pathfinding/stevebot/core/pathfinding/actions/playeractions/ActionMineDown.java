...
public class ActionMineDown extends Action {
    ...
    private abstract static class MineDownActionFactory implements ActionFactory {
        ...
        Result check(Node node, Direction direction) {
            ...
            return Result.valid(
                    direction,
                    NodeCache.get(to),
                    totalTicksTopBreak + ActionCosts.get().STEP_DOWN_STRAIGHT + ActionCosts.get().CONSTANT_BLOCK_BREAK_MOD,
                    modifications
            );
        }
    }

    private abstract static class AbstractMineDownActionFactory extends MineDownActionFactory {
