...
public class ActionBridgeFree extends Action {
    ...
    /**
     * Move into the position required to place the block.
     */
    private ProcState tickPrepare() {
        PlayerUtils.getInput().holdSneak();
        PlayerUtils.getMovement().moveTowards(getTo().getPos(), true);
        if (
                getTo().getPos().equals(PlayerUtils.getPlayerBlockPos()) &&
                        BlockUtils.distToEdge(PlayerUtils.getPlayerPosition(), direction.opposite()) > 0.2
        ) {
    ...
    private abstract static class BrideFreeActionFactory implements ActionFactory {
    ...
    private abstract static class AbstractBrideFreeActionFactory extends BrideFreeActionFactory {
