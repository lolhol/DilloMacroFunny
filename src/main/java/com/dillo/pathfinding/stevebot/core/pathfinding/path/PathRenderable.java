    ...
    for (int i = 0; i < path.getNodes().size() - 1; i++) {
      ...
      for (Modification mod : to.getAction().getModifications()) {
        if (mod instanceof BlockBreakModification) {
          ...
          renderer.drawBoxOpen(
            new Vector3d(breakMod.getPosition().getX(), breakMod.getPosition().getY(), breakMod.getPosition().getZ()),
            Color.RED
          );
        }
        if (mod instanceof BlockPlaceModification) {
          ...
          renderer.drawBoxOpen(
            new Vector3d(placeMod.getPosition().getX(), placeMod.getPosition().getY(), placeMod.getPosition().getZ()),
            Color.YELLOW
          );
        }
      }
