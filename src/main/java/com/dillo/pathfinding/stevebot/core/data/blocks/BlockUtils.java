package com.dillo.pathfinding.stevebot.core.data.blocks;
...
public class BlockUtils {
    ...
    public static boolean canWalkThrough(BlockWrapper block, BaseBlockPos pos) {
        if (
                isLiquid(block) ||
                        WATERLILY == block.getId() ||
                        isDangerous(block) ||
                        ICE == block.getId() ||
                        FROSTED_ICE == block.getId() ||
                        PACKED_ICE == block.getId() ||
                        isDoorLike(block)
        ) {
            return false;
        } else {
    ...
    public static boolean canWalkOn(BlockWrapper block) {
        if (isLiquid(block) || isDangerous(block)) {
            ...
            return (
                    block.isNormalCube() || ICE == block.getId() || FROSTED_ICE == block.getId() || PACKED_ICE == block.getId()
            );
        }
    }
