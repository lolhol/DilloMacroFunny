...
public class GetBlocksForNuker {
  ...
  public static List<BlockPos> filterOutAir(List<BlockPos> blocks) {
    return blocks
      .stream()
      .filter(a -> {
        Block block = BlockUtils.getBlock(a);

        return block != Blocks.air && !isBlockOnRoute(a, block);
      })
      .collect(Collectors.toList());
  }

  public static boolean isBlockOnRoute(BlockPos block, Block blockState) {
    return BlockUtils.isOnBlockInRoute(block) && blockState != Blocks.cobblestone;
  }
