        ...
        if (render) {
            for (int i = 0; i < currentRoute.strucList.size(); i++) {
                try {
                    ...
                    renderStr(String.valueOf(i + 1), currentRoute.strucList.get(i).getX() + 0.5, currentRoute.strucList.get(i).getY() - 1, currentRoute.strucList.get(i).getZ() + 0.5, event.partialTicks, false);
                } catch (Exception e) {
                }
            }
        }
