  ...
  public void fireTransition(T transition) {
    for (TransitionDefinition definition : definitions) {
      if (definition.transition == transition && definition.start == getState()) {
        ...
        transitionListenersAll.forEach(listener ->
          listener.onTransition(definition.start, definition.target, transition)
        );
        transitionListenerMap
          .getOrDefault(transition, Collections.emptyList())
          .forEach(listener -> listener.onTransition(definition.start, definition.target, transition));
        return;
      }
    }
  }
