...
public class ActionEnterWater extends Action {
  ...
  public String getActionNameExp() {
    return (
      this.getActionName() + (Direction.get(getFrom().getPos(), getTo().getPos()).diagonal ? "-diagonal" : "-straight")
    );
  }
  ...
  private abstract static class EnterWaterActionFactory implements ActionFactory {
    ...
    Result checkDiagonal(Node node, Direction direction) {
      ...
      boolean avoid0 =
        BlockUtils.avoidTouching(p0) || BlockUtils.avoidTouching(p0.copyAsFastBlockPos().add(Direction.UP));
      boolean avoid1 =
        BlockUtils.avoidTouching(p1) || BlockUtils.avoidTouching(p1.copyAsFastBlockPos().add(Direction.UP));
  ...
  private abstract static class AbstractEnterWaterActionFactory extends EnterWaterActionFactory {
