...
public class ActionWalk extends Action {
  ...
  public String getActionNameExp() {
    return (
      this.getActionName() +
      (sprint ? "-sprint" : "") +
      (Direction.get(getFrom().getPos(), getTo().getPos()).diagonal ? "-diagonal" : "-straight") +
      (touchesBlock ? "-touches" : "")
    );
  }
  ...
  private abstract static class WalkActionFactory implements ActionFactory {
  ...
  private abstract static class AbstractWalkActionFactory extends WalkActionFactory {
