...
public class VectorUtils {
  ...
  public static ArrayList<BlockPos> getAllTeleportableBlocks(Vec3 vec, float range) {
    ...
    Iterable<BlockPos> blocks = BlockPos.getAllInBox(
      origin.subtract(new Vec3i(range, 16, range)),
      origin.add(new Vec3i(range, 16, range))
    );
    ArrayList<BlockPos> validBlocks = (ArrayList<BlockPos>) StreamSupport
      .stream(blocks.spliterator(), false)
      .filter(blockPos ->
        mc.theWorld.getBlockState(blockPos).getBlock().isCollidable() &&
        mc.theWorld.getBlockState(blockPos).getBlock() != Blocks.carpet &&
        mc.theWorld.getBlockState(blockPos).getBlock() != Blocks.skull &&
        mc.theWorld.getBlockState(blockPos).getBlock() != Blocks.wall_sign &&
        ...
        mc.theWorld
          ...
          .getBlock()
          .getCollisionBoundingBox(mc.theWorld, blockPos, mc.theWorld.getBlockState(blockPos)) !=
        null &&
  ...
  public static boolean isRayTraceableLook(
    final Vec3 position,
    final Vec3 target,
    final BlockPos goal,
    final float range
  ...
  public static Vec3 getVeryAccurateHittableHitVec(final BlockPos pos) {
    ...
    for (int x = 1; x < 9; ++x) {
      for (int y = 1; y < 9; ++y) {
        for (int z = 1; z < 9; ++z) {
          vec =
            new Vec3(pos.getX() + x / 8.0f - 0.0625, pos.getY() + y / 8.0f - 0.0625, pos.getZ() + z / 8.0f - 0.0625);
      ...
      for (int y = 1; y < 5; ++y) {
        for (int z = 1; z < 5; ++z) {
          final Vec3 vec = new Vec3(
            pos.getX() + x / 4.0f - 0.125,
            pos.getY() + y / 4.0f - 0.125,
            pos.getZ() + z / 4.0f - 0.125
          );
  ...
  public static Vec3 getClosestHittableToMiddle(final BlockPos pos) {
    final Vec3 middle = new Vec3(
      pos.getX() + 0.5 + MathUtils.randomFloat() / 4.0f,
      pos.getY() + 0.5 + MathUtils.randomFloat() / 4.0f,
      pos.getZ() + 0.5 + MathUtils.randomFloat() / 4.0f
    );
    double bestDist = 9.9999999E7;
    Vec3 bestHit = null;
    for (int x = 1; x < 6; ++x) {
      for (int y = 1; y < 6; ++y) {
        for (int z = 1; z < 6; ++z) {
          final Vec3 vec = new Vec3(
            (double) (pos.getX() + x / 5.0f - 0.1234f),
            (double) (pos.getY() + y / 5.0f - 0.1234f),
            (double) (pos.getZ() + z / 5.0f - 0.1234f)
          );
          final MovingObjectPosition position = rayTraceLook(vec, 4.5f);
          if (position != null && position.getBlockPos().equals((Object) pos)) {
  ...
  public static ArrayList<Vec3> getAllVeryAccurateHittablePosition(final BlockPos pos) {
    ...
    for (int x = 1; x < 9; ++x) {
      for (int y = 1; y < 9; ++y) {
        for (int z = 1; z < 9; ++z) {
          final Vec3 vec = new Vec3(
            pos.getX() + x / 8.0f - 0.0625,
            pos.getY() + y / 8.0f - 0.0625,
            pos.getZ() + z / 8.0f - 0.0625
          );
          final MovingObjectPosition position = rayTraceLook(vec, 4.5f);
          if (position != null && position.getBlockPos().equals((Object) pos)) {
  ...
  public static ArrayList<Pair<BlockPos, ArrayList<Vec3>>> getAllVisibleWhitelistedBlocksWithAllHittablePos(
    int range,
    ArrayList<IBlockState> whitelist
  ) {
  ...
  public static ArrayList<BlockPos> getAllVisibleWhitelistedBlocks(ArrayList<IBlockState> whitelist) {
    ...
    for (BlockPos blockPos : BlockPos.getAllInBox(new BlockPos(from), new BlockPos(to))) {
      if (
        new Vec3(blockPos).distanceTo(playerVec) < distance &&
        whitelist.contains(BlockUtils.getBlockState(blockPos)) &&
        isHittable(blockPos)
      ) {
  ...
  public static MovingObjectPosition calculateInterceptAABBLook(
  ...
  public static MovingObjectPosition fastRayTrace(Vec3 vec31, final Vec3 vec32, final boolean hitLiquid) {
    ...
    if (block.canCollideCheck(iblockstate, hitLiquid)) {
      final MovingObjectPosition movingobjectposition = block.collisionRayTrace(
        (World) BlockUtils.getWorld(),
        blockpos,
        vec31,
        vec32
      );
    ...
    while (k2-- >= 0) {
      ...
      final MovingObjectPosition movingobjectposition2 = block2.collisionRayTrace(
        (World) BlockUtils.getWorld(),
        blockpos,
        vec31,
        vec32
  ...
  private static boolean rayTraceable(Vec3 vec31, final Vec3 vec32, final BlockPos goal) {
    ...
    while (k2-- >= 0) {
      ...
      if (movingobjectposition2 != null) {
        return (
          (blockpos == goal || (l == goal.getX() && i2 == goal.getY() && j2 == goal.getZ())) &&
          BlockUtils.getBlockState(blockpos.offset(enumfacing)).getBlock() == Blocks.air
        );
      }
    }
    return false;
