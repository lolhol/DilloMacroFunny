package com.dillo.Pathfinding.baritone.automine.player;
...
public class Rotation {
  ...
  public void initAngleLock(BlockPos block, int time) {
    Tuple<Float, Float> angles = AngleUtils.getRotation(
  ...
  public void update() {
    if (System.currentTimeMillis() <= endYawTime) {
      mc.thePlayer.rotationYaw =
        shouldRotateClockwise(start.left, target.left)
          ? start.left + interpolateYaw(difference.left)
          : start.left - interpolateYaw(difference.left);
    } else if (!completed) {
