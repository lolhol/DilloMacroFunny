...
public class NpcUtil {
  ...
  private static final Pattern healthPattern = Pattern.compile(
    "(?:§8[§7Lv(\\d)§8])?\\s*(?:§c)?(.+)(?:§r)? §[ae]([\\dBMk]+)§c❤"
  );
  private static final Pattern healthPattern2 = Pattern.compile(
    "(?:§8[§7Lv(\\d)§8])?\\s*(?:§c)?(.+)(?:§r)? §[ae]([\\dBMk]+)§f/§[ae]([\\dBMk]+)§c❤"
  );

  public static Entity getEntityCuttingOtherEntity(Entity e, Class<?> entityType) {
    List<Entity> possible = Minecraft
      .getMinecraft()
      .theWorld.getEntitiesInAABBexcluding(
        e,
  ...
  public static boolean entityIsTargeted(Entity entity) {
    ...
    for (Entity e : entityList) {
      ...
      MovingObjectPosition movingObjectPosition = entityBoundingBox.calculateIntercept(
        eyesPos,
        eyesPos.addVector(lookVec.xCoord * reach, lookVec.yCoord * reach, lookVec.zCoord * reach)
      );
