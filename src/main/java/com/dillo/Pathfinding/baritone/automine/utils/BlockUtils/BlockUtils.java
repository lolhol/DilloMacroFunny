package com.dillo.Pathfinding.baritone.automine.utils.BlockUtils;

import static com.dillo.Pathfinding.baritone.automine.utils.AngleUtils.*;
import static com.dillo.Pathfinding.baritone.automine.utils.PlayerUtils.AnyBlockAroundVec3;

import com.dillo.Pathfinding.baritone.automine.utils.AngleUtils;
import com.dillo.Pathfinding.baritone.automine.utils.PlayerUtils;
...
public class BlockUtils {
  ...
  private static final Comparator<Vec3> sortVec3ByRotation = (a, b) -> {
    if (
      getRotationCost(getRotation(a).getFirst(), getRotation(a).getSecond()) ==
      getRotationCost(getRotation(b).getFirst(), getRotation(b).getSecond())
    ) return 0;
    return (
        getRotationCost(getRotation(a).getFirst(), getRotation(a).getSecond()) >
        getRotationCost(getRotation(b).getFirst(), getRotation(b).getSecond())
  ...
  private boolean isTitanium(BlockPos pos) {
    ...
    return (
      ...
      (state.getValue(BlockStone.VARIANT)).equals(BlockStone.EnumType.DIORITE_SMOOTH)
    );
  }
  ...
  private static final LoadingCache<BlockPos, Block> blockCache = CacheBuilder
    .newBuilder()
    .expireAfterWrite(3L, TimeUnit.SECONDS)
    .build(
      new CacheLoader<BlockPos, Block>() {
  ...
  public static List<BlockPos> findBlockInCube(
    int boxDiameter,
    ArrayList<BlockPos> forbiddenBlockPos,
    int minY,
    int maxY,
    ...
    return findBlock(
      new Box(-boxDiameter / 2, boxDiameter / 2, -boxDiameter / 2, boxDiameter / 2, -boxDiameter / 2, boxDiameter / 2),
      forbiddenBlockPos,
      minY,
      maxY,
      blockDataList
  ...
  public static List<BlockPos> findBlockInCube(
    ...
    return findBlock(
      ...
      minY,
      maxY,
      new ArrayList<>(Arrays.asList(requiredBlocks))
    );
  }
  ...
  public static List<BlockPos> findBlockInCube(
    ...
    int minY,
    int maxY,
    ArrayList<BlockData<?>> requiredBlocks
  ) {
    return findBlock(
      new Box(-boxDiameter / 2, boxDiameter / 2, -boxDiameter / 2, boxDiameter / 2, -boxDiameter / 2, boxDiameter / 2),
      forbiddenBlockPos,
  ...
  public static List<BlockPos> findBlock(
    Box box,
    ArrayList<BlockPos> forbiddenBlockPos,
    int minY,
    int maxY,
  ...
  public static List<BlockPos> findBlock(
    ...
    int minY,
    int maxY,
    ArrayList<BlockData<?>> requiredBlock
  ) {
    List<BlockPos> foundBlocks = new ArrayList<>();
    if (forbiddenBlockPos != null && !forbiddenBlockPos.isEmpty()) forbiddenBlockPos.forEach(System.out::println);
        ...
        for (int k = 0; k <= Math.abs(searchBox.dz_bound2 - searchBox.dz_bound1); k++) {
          ...
          currentBlock =
            (
              getPlayerLoc()
                .add(
                  i + Math.min(searchBox.dx_bound2, searchBox.dx_bound1),
                  j + Math.min(searchBox.dy_bound2, searchBox.dy_bound1),
                  k + Math.min(searchBox.dz_bound2, searchBox.dz_bound1)
                )
            );
          ...
          if (
            requiredBlock
              ...
              .anyMatch(blockData -> {
                ...
                if (blockData.requiredBlockStateValue instanceof EnumDyeColor) return (
                  block.getMetaFromState(mc.theWorld.getBlockState(finalCurrentBlock)) ==
                  ((EnumDyeColor) blockData.requiredBlockStateValue).getMetadata()
                );
                if (blockData.requiredBlockStateValue instanceof BlockStone.EnumType) return (
                  block.getMetaFromState(mc.theWorld.getBlockState(finalCurrentBlock)) ==
                  ((BlockStone.EnumType) blockData.requiredBlockStateValue).getMetadata()
                );
            ...
            if (
              forbiddenBlockPos != null && !forbiddenBlockPos.isEmpty() && forbiddenBlockPos.contains(currentBlock)
            ) continue;
            if (currentBlock.getY() > maxY || currentBlock.getY() < minY) continue;
    ...
    foundBlocks.sort(
      Comparator.comparingDouble(b ->
        MathUtils.getDistanceBetweenTwoBlock(b, BlockUtils.getPlayerLoc().add(0, 1.62d, 0))
      )
    );
    return foundBlocks;
  }
  ...
  public static boolean isBlock(BlockData<?> blockData, BlockPos blockPos) {
    ...
    if (blockData.requiredBlockStateValue instanceof EnumDyeColor) return (
      block.getMetaFromState(blockState) == ((EnumDyeColor) blockData.requiredBlockStateValue).getMetadata()
    );
    if (blockData.requiredBlockStateValue instanceof BlockStone.EnumType) return (
      block.getMetaFromState(blockState) == ((BlockStone.EnumType) blockData.requiredBlockStateValue).getMetadata()
    );
    return false;
  }

  public static List<BlockPos> findBlockFromState(
    Box searchBox,
    ArrayList<BlockPos> forbiddenBlockPos,
    int minY,
    int maxY,
    ArrayList<IBlockState> requiredBlock
  ) {
    List<BlockPos> foundBlocks = new ArrayList<>();
    ...
    for (BlockPos blockPos : BlockPos.getAllInBox(from, to)) {
      if (
        requiredBlock.contains(BlockUtils.getBlockState(blockPos)) &&
        new Vec3(blockPos).distanceTo(PlayerUtils.playerEyePosVec()) < 4 &&
        !forbiddenBlockPos.contains(blockPos) &&
        blockPos.getY() < maxY &&
        blockPos.getY() > minY
      ) {
        if (
          AngleUtils.getRotationDifference(getBlockCenter(blockPos)) <=
          200 *
          (1.0f + (4 - new Vec3(blockPos).distanceTo(PlayerUtils.playerEyePosVec())) / 5) ||
  ...
  public static boolean scanBox(
    OffsetBox box,
    List<Block> blocksAllowed,
    List<Block> blocksForbidden,
    float closetPlayerYaw
  ) {
    BlockPos check;
    ...
    for (int i = -1; i < Math.abs(box.right_bound1 - box.right_bound2); i++) {
      for (int j = -1; j < Math.abs(box.up_bound1 - box.up_bound2); j++) {
        for (int k = -1; k < Math.abs(box.front_bound1 - box.front_bound2); k++) {
          check =
            BlockUtils.getRelativeBlockPos(
          ...
          if (
            blocksAllowed != null &&
            (!blocksAllowed.contains(getBlock(check)) || !blocksAllowed.contains(getBlock(check.up())))
          ) return true;
          if (
            blocksForbidden != null &&
            (blocksForbidden.contains(getBlock(check)) || !blocksForbidden.contains(getBlock(check.up())))
          ) return true;
        }
      }
    }
  ...
  public static BlockPos getRelativeBlockPos(
    float rightOffset,
    float upOffset,
    float frontOffset,
    float closetPlayerYaw
  ...
  public static boolean isAxisAdjacent(BlockPos bp) {
    return (
      onTheSameXZ(bp.east(), BlockUtils.getPlayerLoc()) ||
      onTheSameXZ(bp.south(), BlockUtils.getPlayerLoc()) ||
      onTheSameXZ(bp.north(), BlockUtils.getPlayerLoc()) ||
      onTheSameXZ(bp.west(), BlockUtils.getPlayerLoc()) ||
      onTheSameXZ(bp, BlockUtils.getPlayerLoc())
    );
  }

  public static boolean isAStraightLine(BlockPos b1, BlockPos b2, BlockPos b3) {
    if ((b1.getX() - b2.getX()) == 0 || (b2.getX() - b3.getX()) == 0 || (b1.getX() - b3.getX()) == 0) return (
      (b1.getX() - b2.getX()) == 0 &&
      (b2.getX() - b3.getX()) == 0 &&
      (b1.getX() - b3.getX()) == 0 &&
      b1.getY() == b2.getY() &&
    ...
    return (
      (
        (b1.getZ() - b2.getZ()) / (b1.getX() - b2.getX()) == (b2.getZ() - b3.getZ()) / (b2.getX() - b3.getX()) &&
        (b1.getZ() - b2.getZ()) / (b1.getX() - b2.getX()) == (b1.getZ() - b3.getZ()) / (b1.getX() - b3.getX())
      ) &&
      b1.getY() == b2.getY() &&
      b2.getY() == b3.getY()
    );
  ...
  public static boolean fitsPlayer(BlockPos groundBlock) {
    return (
      canWalkOn(getBlock(groundBlock)) &&
  ...
  public static boolean isAdjacentXZ(BlockPos b1, BlockPos b2) {
    return (
      (b1.getX() == b2.getX() && Math.abs(b1.getZ() - b2.getZ()) == 1) ||
  ...
  public static boolean canMineBlock(BlockPos b) {
    return !BlockUtils
      .getAllVisibilityLines(b, mc.thePlayer.getPositionVector().add(new Vec3(0, mc.thePlayer.getEyeHeight(), 0)))
      .isEmpty();
  }
  ...
  public static boolean hasBlockInterfere(BlockPos b1, BlockPos b2) {
    for (BlockPos pos : getAllBlocksInLine2d(b1, b2)) {
      if (!BlockUtils.isPassable(pos)) return true;
    }
    return false;
  }
  ...
  public static ArrayList<BlockPos> GetAllBlocksInline3d(BlockPos pos1, BlockPos pos2) {
    ...
    Vec3 startPos = new Vec3(
      pos1.getX() + 0.5,
      pos1.getY() + 1 + mc.thePlayer.getDefaultEyeHeight() - 0.125,
      pos1.getZ() + 0.5
    );
  ...
  public static Vec3 getCloserVisibilityLine(BlockPos pos, int randomness) {
    ArrayList<Vec3> lines = getAllVisibilityLines(
      pos,
      mc.thePlayer.getPositionVector().add(new Vec3(0, mc.thePlayer.getEyeHeight(), 0))
    );
    if (lines.isEmpty()) {
      return null;
    }
  ...
  private static float getRotationCost(float yaw, float pitch) {
    return (
      Math.abs(getAngleDifference(getActualRotationYaw(yaw), getActualRotationYaw())) +
      Math.abs(mc.thePlayer.rotationPitch - pitch)
    );
  }
  ...
  public static ArrayList<Vec3> getAllVisibilityLines(BlockPos pos, Vec3 fromEye, boolean lowerY) {
    ...
    float spaceFromEdge = lowerY ? 0.1f : 0.1f;
      ...
      for (float y = pos.getY() + spaceFromEdge; y <= pos.getY() + (1f - spaceFromEdge); y += accuracy) {
        for (float z = pos.getZ() + spaceFromEdge; z <= pos.getZ() + (1f - spaceFromEdge); z += accuracy) {
          ...
          if (movingObjectPosition != null) {
            ...
            if (obj.equals(test)) lines.add(target);
          }
        }
      }
    }
