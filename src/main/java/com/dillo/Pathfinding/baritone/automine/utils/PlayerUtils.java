...
public class PlayerUtils {
  ...
  public static boolean hasStoppedMoving() {
    return (
      mc.thePlayer.posX - mc.thePlayer.lastTickPosX == 0 &&
  ...
  public static int getItemInHotbarWithBlackList(
    boolean returnErrorCode,
    String blackListedLore,
    final String... itemName
  ) {
    for (int i = 0; i < 8; ++i) {
      final ItemStack is = mc.thePlayer.inventory.getStackInSlot(i);
      if (blackListedLore != null && containsLore(is, blackListedLore)) continue;
      for (String s : itemName) {
        if (is != null && StringUtils.stripControlCodes(is.getDisplayName()).contains(s)) {
          return i;
        }
  ...
  public static int getItemInHotbarFromLore(boolean returnErrorCode, final String lore) {
    for (int i = 0; i < 8; ++i) {
      if (containsLore(mc.thePlayer.inventory.mainInventory[i], lore)) return i;
    }
    return returnErrorCode ? -1 : 0;
  }
  ...
  public static boolean notAtCenter(float rotationYawAxis) {
    return !(
      (rotationYawAxis % 180 == 0)
        ? Math.abs(mc.thePlayer.posX) % 1 >= 0.3f && Math.abs(mc.thePlayer.posX) % 1 <= 0.7f
        : Math.abs(mc.thePlayer.posZ) % 1 >= 0.3f && Math.abs(mc.thePlayer.posZ) % 1 <= 0.7f
    );
  }

  public static boolean containsLore(ItemStack item, String lore) {
    ArrayList<String> lores = new ArrayList<>();
    if (item != null) lores = getItemLore(item);
