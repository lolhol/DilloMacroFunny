...
import com.dillo.Pathfinding.baritone.automine.calculations.exceptions.ChunkLoadException;
import com.dillo.Pathfinding.baritone.automine.logging.Logger;
import com.dillo.Pathfinding.baritone.automine.movement.Moves;
...
import com.dillo.Pathfinding.baritone.automine.structures.Node;
...
public class AStarCalculator {
  ...
  public LinkedList<BlockNode> calculatePath(
  ...
  private void checkNode(Moves move, Node searchNode, Node currentNode, BlockPos endingBlockPos)
    ...
    if (pathFinderBehaviour.getForbiddenMiningBlocks() != null) {
      switch (move) {
        ...
        case ASCEND_WEST:
          if (
            pathFinderBehaviour.getForbiddenMiningBlocks().contains(BlockUtils.getBlock(currentNode.pos.up(2)))
          ) return;
          break;
        case DESCEND_EAST:
        case DESCEND_NORTH:
        ...
        case DESCEND_WEST:
          if (
            pathFinderBehaviour.getForbiddenMiningBlocks().contains(BlockUtils.getBlock(searchNode.pos.up(2)))
          ) return;
          break;
      }
      if (
        pathFinderBehaviour.getForbiddenMiningBlocks().contains(BlockUtils.getBlock(searchNode.pos)) ||
        pathFinderBehaviour.getForbiddenMiningBlocks().contains(BlockUtils.getBlock(searchNode.pos.up()))
      ) return;
    }
    ...
    if (pathFinderBehaviour.getAllowedMiningBlocks() != null) {
      switch (move) {
        ...
        case ASCEND_WEST:
          if (
            !pathFinderBehaviour.getAllowedMiningBlocks().contains(BlockUtils.getBlock(currentNode.pos.up(2)))
          ) return;
          break;
        case DESCEND_EAST:
        case DESCEND_NORTH:
        case DESCEND_SOUTH:
        case DESCEND_WEST:
          if (!pathFinderBehaviour.getAllowedMiningBlocks().contains(BlockUtils.getBlock(searchNode.pos.up(2)))) return;
          break;
      }
      if (
        !pathFinderBehaviour.getAllowedMiningBlocks().contains(BlockUtils.getBlock(searchNode.pos)) ||
        !pathFinderBehaviour.getAllowedMiningBlocks().contains(BlockUtils.getBlock(searchNode.pos.up()))
      ) return;
    }
    ...
    switch (move) {
      ...
      case DIAGONAL_SOUTHWEST:
        ...
        if (
          !BlockUtils.fitsPlayer(searchNode.pos.down()) ||
          !BlockUtils.fitsPlayer(currentNode.pos.down()) ||
          !BlockUtils.fitsPlayer(block3.down()) ||
          !BlockUtils.fitsPlayer(block4.down())
        ) {
          return;
        }
        break;
    ...
    if (mode == PathMode.MINE) {
      if (goalNode.parentNode != null && goalNode.parentNode.pos != null) {
        ...
        if (goalNode.parentNode.pos.getY() > goalNode.pos.getY()) {
          if (!BlockUtils.isPassable(goalNode.pos.up())) blocksToMine.add(
            new BlockNode(goalNode.pos.up(), getBlockType(goalNode.pos.up()))
          );
          if (
            !BlockUtils.isPassable(goalNode.pos.up(2)) && getBlockType(goalNode.pos.up(2)) == BlockType.WALK
          ) blocksToMine.add(new BlockNode(goalNode.pos.up(2), getBlockType(goalNode.pos.up(2))));
        } else if (
          goalNode.parentNode.pos.getY() == goalNode.pos.getY() &&
    ...
    if (currentTrackNode != null && currentTrackNode.parentNode != null) {
      do {
        ...
        if (currentTrackNode.parentNode.pos.getY() > currentTrackNode.pos.getY()) { // going down
          ...
          if (
            !BlockUtils.isPassable(currentTrackNode.pos.up(2)) &&
            (
              (!formerNode.pos.equals(goalNode.pos) && !BlockUtils.isPassable(currentTrackNode.pos)) ||
              BlockUtils.isPassable(currentTrackNode.pos)
            )
          ) {
            blocksToMine.add(e);
          }
      ...
      if (
        (blocksToMine.getLast()).getPos().getY() >= (int) mc.thePlayer.posY + 1 &&
        (blocksToMine.get(blocksToMine.size() - 2)).getPos().getY() >= (int) mc.thePlayer.posY + 1
      ) {
