package com.dillo.Pathfinding.baritone.automine.movement;
...
import com.dillo.Pathfinding.baritone.automine.player.Rotation;
...
public class PathExecutor {
  ...
  public void onTickEvent(TickEvent.ClientTickEvent event) {
    ...
    if (
      blocksInPath.isEmpty() ||
      (BlockUtils.isPassable(blocksInPath.getFirst().getPos()) && blocksInPath.getFirst().getType() == BlockType.MINE)
    ) {
      if (
        !shouldGoToFinalBlock ||
        (!finishedPath.isEmpty() && BlockUtils.getPlayerLoc().equals(finishedPath.getLast().getPos()))
      ) {
        disable();
        return;
      }
    ...
    switch (currentState) {
      case WALKING:
        BlockPos targetWalkBlock = (blocksInPath.isEmpty() || blocksInPath.getLast().getType() == BlockType.MINE)
          ? finishedPath.getLast().getPos()
          : blocksInPath.getLast().getPos();

        if (finishedPath.isEmpty()) { // initialize
          float reqYaw = AngleUtils.getRequiredYawCenter(targetWalkBlock);
          if (rotator.rotating) return;
          if (
            AngleUtils.getAngleDifference(reqYaw, AngleUtils.getActualRotationYaw()) > 1f ||
            Math.abs(mc.thePlayer.rotationPitch - 0) > 1f
          ) {
      ...
      case MINING:
        ...
        if (lookVector != null) rotator.initAngleLock(
          AngleUtils.getRotation(lookVector).getFirst(),
          AngleUtils.getRotation(lookVector).getSecond(),
          config.getRotationTime()
        );
        break;
    }
  ...
  public void onRenderEvent(RenderWorldLastEvent event) {
    ...
    if (rotator.rotating) rotator.update();
  }
        ...
        if (!blocksInPath.isEmpty()) {
          for (int i = 0; i < blocksInPath.size(); i++) {
            mc.fontRendererObj.drawString(
              blocksInPath.get(i).getPos().toString() + " " + blocksInPath.get(i).getType().toString(),
              5,
              5 + 10 * i,
              -1
            );
    ...
    switch (currentState) {
      case WALKING:
        if (finishedPath.getLast().getType() == BlockType.WALK) {
          if (!(blocksInPath.getLast().getType() == BlockType.MINE)) return;
        }
        if (finishedPath.getLast().getType() == BlockType.MINE) {
          if (
            BlockUtils.fitsPlayer(finishedPath.getLast().getPos().down()) &&
            !BlockUtils.onTheSameXZ(finishedPath.getLast().getPos(), BlockUtils.getPlayerLoc())
          ) return;
        }
        currentState = PlayerState.MINING;
        break;
      case MINING:
        if (
          blocksInPath.getLast().getType() == BlockType.MINE &&
          shouldWalkTo(finishedPath.getLast().getPos(), blocksInPath.getLast().getPos())
        ) currentState = PlayerState.WALKING;
        break;
    }
  }
  ...
  private boolean shouldRemoveFromList(BlockNode lastBlockNode) {
    if (lastBlockNode.getType() == BlockType.MINE) return (
      BlockUtils.isPassable(lastBlockNode.getPos()) ||
      BlockUtils.getBlock(lastBlockNode.getPos()).equals(Blocks.bedrock)
    ); else return (
      BlockUtils.onTheSameXZ(lastBlockNode.getPos(), BlockUtils.getPlayerLoc()) ||
      !BlockUtils.fitsPlayer(lastBlockNode.getPos().down())
    );
  }
  ...
  private boolean shouldWalkTo(BlockPos blockPos, BlockPos nextBlockPos) {
    return (
      (
        (blockPos.getY() > Math.round(mc.thePlayer.posY) && BlockUtils.isPassable(BlockUtils.getPlayerLoc().up(2))) || // It is possible to walk to there (no headhitters)
    ...
    ); // optimization, prevent it from jumping on blocks;
  }
}
