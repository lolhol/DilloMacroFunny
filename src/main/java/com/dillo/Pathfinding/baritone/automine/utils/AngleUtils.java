...
public class AngleUtils {
  ...
  public static float getRequiredYaw(double deltaX, double deltaZ) {
    if (deltaX == 0 && deltaZ < 0) return -180f; // special case

    return (
      (float) (Math.atan(-deltaX / deltaZ) * 180 / Math.PI) +
  ...
  public static Pair<Float, Float> getYawAndPitch(final Vec3 vec, final boolean predict) {
    ...
    return Pair.of(
      AngleUtils.wrapAngleTo180((double) Math.toDegrees(Math.atan2(diffZ, diffX)) - 90F),
  ...
  public static float getActualYawFrom360(float yaw360) {
    ...
    if (mc.thePlayer.rotationYaw > yaw360) {
      while (mc.thePlayer.rotationYaw - currentYaw < 180 || mc.thePlayer.rotationYaw - currentYaw > 0) {
        if (
          Math.abs(currentYaw + 360 - mc.thePlayer.rotationYaw) < Math.abs(currentYaw - mc.thePlayer.rotationYaw)
        ) currentYaw = currentYaw + 360; else break;
      }
    }
    if (mc.thePlayer.rotationYaw < yaw360) {
    ...
    if (blockSidesNotCovered.size() > 0 && !lookAtCenterBlocks.contains(BlockUtils.getBlock(blockPos))) {
      ...
      for (BlockUtils.BlockSides blockSide : blockSidesNotCovered) {
        ...
        switch (blockSide) {
          case posX:
            tempCost =
              MathUtils.getDistanceBetweenTwoPoints(
                mc.thePlayer.posX,
                mc.thePlayer.posY + 1.62d,
                mc.thePlayer.posZ,
                blockPos.getX() + 0.5d,
                blockPos.getY(),
                blockPos.getZ()
