    ...
    public static boolean blocksAbove(BlockPos baseBlock, Block[] blockList, BlockStone.EnumType[] stoneParameters) {
        ...
        for (int i = 0; i < blockList.length; i++) {
            if (ids.mc.theWorld.getBlockState(new BlockPos(x, y + i, z)).getBlock() == blockList[i]) {
                if (
                        stoneParameters[i] != null &&
                                (ids.mc.theWorld.getBlockState(new BlockPos(x, y + i, z)).getBlock() == Blocks.stone)
                ) {
                    if (
                            ids.mc.theWorld.getBlockState(new BlockPos(x, y + i, z)).getValue(BlockStone.VARIANT) != stoneParameters[i]
                    ) return false;
                }
            } else {
                return false;
