...
public class FontRender extends FontCore implements Comparator<String> {
  ...
  FontCore.CharData[] boldChars = new FontCore.CharData[256], italicChars =
    new FontCore.CharData[256], boldItalicChars = new FontCore.CharData[256];
    ...
    for (int index = 0; index < text.length(); index++) {
      ...
      if (character == '\u00a7') {
        ...
        if (colorIndex < 16) {
          ...
          GlStateManager.color(
            (float) (colorcode >> 16 & 255) / 255f,
            (float) (colorcode >> 8 & 255) / 255f,
            (float) (colorcode & 255) / 255f,
            alpha
          );
  ...
  private void setupBoldItalicIDs() {
    this.texBold =
      this.setupTexture(this.font.deriveFont(Font.BOLD), this.antiAlias, this.fractionalMetrics, this.boldChars);
    this.texItalic =
      this.setupTexture(this.font.deriveFont(Font.ITALIC), this.antiAlias, this.fractionalMetrics, this.italicChars);
    this.texItalicBold =
      this.setupTexture(
